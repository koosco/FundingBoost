...
class ReviewServiceTest {
    ...
    @BeforeEach
    void init() {
        mockReview = spy(new Review(5, "content", mockMember, mockItem, List.of("uploaded/test.jpg")));
        doReturn(mockReviewId).when(mockReview).getId();
        when(memberService.findMember(memberId)).thenReturn(mockMember);
        when(orderService.existsByMemberId(memberId)).thenReturn(true);
    ...
    void givenIdAndDtoAndFiles_whenCreateReview_thenSaveReview() {
        // given
        CreateReviewRequestDto requestDto = new CreateReviewRequestDto(itemId, "content", 5);
        ...
        verify(orderService, times(1)).existsByMemberId(memberId);
